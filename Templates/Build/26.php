<?php
if (!isset($_SESSION['time_p']) || !is_numeric($_SESSION['time_p'])) {
...
if (isset($_POST) && $_GET['action'] == 'change_capital' && $village->natar == 0) {
    $pass = $_POST['pass'] ?? '';
    ...
    if($data['password'] == md5($pass.mb_convert_case($session->username,MB_CASE_LOWER,"UTF-8"))) {
            ...
            if(count($data1)){
                ...
                if($data2['vref'] != $village->wid) {
                    ...
                    $database->query("DELETE FROM training where unit > 60 and `vref`='".$data1['wref']."'"); //удаляем тренировки для бк
                    $database->query("DELETE FROM bdata where `level` > 10 and `field` <= 18 and `wid`='".$data1['wref'] ."'"); //удаляем тренировки для бк
        ...
        $error = '<br/><font color="red">' . (defined('dvrc0') ? dvrc0 : 'Error message') . '</font><br/>';
