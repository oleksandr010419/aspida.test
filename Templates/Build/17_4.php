<?php if($session->goldclub == 1 && count($session->villages) > 1) {
    ...
    if(isset($_GET['create']) && $session->gold > 1){
        ...
        <table id="npc" cellpadding="1" cellspacing="1">
            ...
            if(count($routes) == 0) {
                ...
                foreach($routes as $route){
                    ?>
                    <tr>
                        <th><a href="build.php?id=<?=$id?>&t=4&action=delRoute&routeid=<?php echo $route['id']; ?>"><img class="del" src="img/x.gif" alt="delete" title="<?=notice24?>"></a></th>
                        <th>
                            <?php
        ...
        <br>
        <div class="options">
            <a class="arrow" href="build.php?gid=17&t=4&create"> <?=rinok49?></a>
        </div>
