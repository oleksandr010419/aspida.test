...
$randomaim = $database->checkArtefactsEffects($session->uid, $village->wid, 7);
...
if (!isset($process['t4']) || $process['t4'] == '') {
    ...
    $t4 = $process['t4'];
    if ($session->tribe == 1 || $session->tribe == 2 || $session->tribe == 4 || $session->tribe == 5 || $session->tribe == 6) {
        $scout = 1;
    }
}
...
<form method="post" action="a2b.php">
    <table id="short_info" cellpadding="1" cellspacing="1">
        <tbody>
            <tr>
                ...
                <td><a href="karte.php?d=<?php
                    $d = $database->getBaseID($coor['x'], $coor['y']);
                    echo $d . "&c=" . $generator->getMapCheck($d);
                    ?>"><?php echo $process[1]; ?> (<?php echo $coor['x']; ?>|<?php echo $coor['y']; ?>)</a></td>
            </tr>
            <tr>
        ...
        <?php if ($process['c'] == 1) {
            ...
            <tbody class="options">
                <tr>
                    ...
                    ?>"><input class="radio" name="spy" value="1" checked="checked" type="radio"><?php echo OTPRAV10; ?>
                        <input class="radio" name="spy" value="2" type="radio"> <?php echo OTPRAV11; ?>                                           </td>
                </tr>
            </tbody>
        <?php } ?>
    ...
    <?php if ($process['c'] == '3') { ?><tbody class="cata">
                                ...
                                if ($br_tm[0] < time()) {
									if($randomaim===1){
										...
										<?php if ($pc >= 10) { ?>
												...
												<option value="41"><?php echo $lang['buildings'][41]; ?></option>
												<option value="45"><?php echo $lang['buildings'][45]; ?></option>
											</optgroup>
										<?php } ?>
										<?php if ($pc >= 10) { ?>
										...
										<?php } ?>
									<?php } else if($randomaim!=6){?>
									<option value="27"><?php echo $lang['buildings'][27]; ?></option>
									<?php }?>
					...
					<span class="info"> <?php echo OTPRAV14; ?></span>
                    </td>
                    </tr>
                    </tbody>
                    <?php
                ...
                <tr>
                    ...
                    <td colspan="11">
                        <select name="artefact" class="dropdown">
                            <?php
                            foreach ($arti as $artefact) {
                                include ('Templates/Build/27_artefact.php');
                                ?>
                                <option value="<?php echo $artefact['id']; ?>"><?php echo $name; ?></option>
                <?php } ?>
                        </select></td>
                </tr>
    <?php
...
//find slowest unit.
    ...
    if ($protect > time() && $process[2] != 2 && $session->protect == 1) {
    ...
    <button type="submit" value="ok" name="s1" id="btn_ok" class="green ">
        <div class="button-container addHoverClick">
            ...
            <div class="button-content"><?= punktxuev3_1 ?></div></div></button>
</form>