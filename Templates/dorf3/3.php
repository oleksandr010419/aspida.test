...
<thead><tr><th colspan="8"><?=len0?></th></tr>
<tr><td><?=len1?></td>
...
<td><img class="r5" src="img/x.gif" title="Crop Consumption" alt="Crop"></td>
<td><img class="clock" src="img/x.gif" title="Clock" alt="Clock"></td>
</tr></thead><tbody>
<?php
if(!isset($timer)){
...
foreach($session->villages as $vid){
    ...
    $consumption -= $database->getUpkeep($database->getAllUnits($vid),$session->tribe,$database->getResourceLevel($vid));
	
	$prod_crop -= $consumption;
	...
	echo '
		...
		<td class="crop" title="Total">'.$consumption.'</td>
