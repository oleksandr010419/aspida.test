...
$to['x'] = $basearray['x'];
$to['y'] = $basearray['y'];
$distance = round(hypot(($to['x']-$from['x']+1201)%801 -400,($to['y']-$from['y']+1201)%801 -400),1);
if($basearray['type_of']!='' && $basearray['oasistype']==0){
    ?>
	<div id="tileDetails">
    <h1 class="titleInHeader"><?=logint43?>
	<span class="coordinates coordinatesWrapper ss"><span class="coordinateX"><?php echo "(".$basearray['x'].""; ?></span><span class="coordinatePipe">|</span><span class="coordinateY"><?php echo "".$basearray['y'].")"; ?></span><span class="clear">&nbsp;</span></span>
	
	</h1>
    <div id="tileDetails" class="landscape landscape-<?=$basearray['type_of']?>">
        <div class="detailImage">
            <div class="options">
				<div class="option"><b>Distance: <?php echo $distance;?> fields</b></div> 
...
$query = count($result);
$surroundingQuery="toWref = ".$toWref." ORDER BY time DESC Limit 5";
$surrounding =  $database->getinfoforndata($what);
?>
...
<h1><?php if($basearray['fieldtype']!=0){
            ...
            echo $oasisname; 	?>
			<span class="coordinates coordinatesWrapper"><span class="coordinateX"><?php echo "(".$basearray['x'].""; ?></span><span class="coordinatePipe">|</span><span class="coordinateY"><?php echo "".$basearray['y'].")"; ?></span></span>
			<?php
            ...
            <?php if(!$basearray['occupied']) { ?>
                <div class="option">
                    ...
                    if($village->unitarray['u10'] >= 3 AND $enough_cp) {
                        $test = "<a class=\"a arrow\" href=\"a2b.php?id=".$getd."&amp;s=1\">".logint411."</a>";
                    ...
                    echo ($basearray['fieldtype']==0)?
                        ($village->resarray['f39']==0)?
                            ...
                            "<a class=\"a arrow\" target=\"_top\" href=\"a2b.php?z=".$getd."\"> ".KAR9."</a>" :
                        "$test"
                    ?>
                </div>
            <?php }
                ...
                <div class="option">
                    ...
                    } else if((($uinfo['regtime']+PROTECTION) < time() || $uinfo['protect']==0) || $basearray['owner']==$session->uid) {
                        echo $village->resarray['f39']? "<a class=\"a arrow\" target=\"_top\" href=\"a2b.php?z=".$getd."\"> ".KAR9."</a>" : "<span class=\"a arrow disabled\">".KAR9." (".KAR8.")</span>";
                    } else {
                        echo "<span class=\"a arrow disabled\">".KAR9."<br /> (".KAR10." <span id=\"timer1\">".$generator->getTimeFormat($uinfo['regtime']+PROTECTION-time())."</span>)</span>";
                    }
                    ?>
                </div>
                <div class="option">
                    ...
                    if($uinfo['access']=='0' || $uinfo['access']=='8' || $uinfo['access']=='9') {
                        ...
                        echo $database->getTypeLvlFdata(17,$village->resarray)? "<a class=\"a arrow\" target=\"_top\" href=\"build.php?z=".$getd."&id=" . $database->getTypeField(17,$village->resarray) . "\"> ".KAR11."</a>" : "<span class=\"a arrow disabled\"> ".KAR11." (".KAR17.")</span>";
                    }
                    ?>
                </div>
				...
				<div class="option">
					<?php if($session->goldclub && $session->right['s4']) { 
						echo "<a class=\"a arrow\" target=\"_top\" href=\"build.php?id=39&t=99&action=showSlot&lid=1&sort=distance&direction=asc&x=".$basearray['x']."&y=".$basearray['y']."\">Add to farm list</a>";
					 } else { 
						echo "<a class=\"a arrow\" target=\"_top\" href=\"#\" onclick=\"window.fireEvent('startPaymentWizard', {}); this.blur(); return false;\">Add to farm list</a>";
					 } ?>
				</div>
            <?php }else if ($basearray['occupied']==1 && $basearray['oasistype']!=0 && $basearray['wref'] != $_SESSION['wid']) { ?>
                <div class="option">
                    <?php
                    echo $village->resarray['f39']? "<a class=\"a arrow\" target=\"_top\" href=\"a2b.php?z=".$d."\">".logint419."</a>" : "<span class=\"a arrow disabled\" title=\"(build rally point)\"><?=logint419?></span>";
                    ?>
                </div>
            <?php } ?>			
        </div>
...
<div id="map_details">
<?php if($uinfo['id']!=2  && !$basearray['fieldtype']){ ?>
...
<?php  if($basearray['fieldtype'] == 0 && !$oasis['conqured']) {
    ...
    <table cellpadding="0" cellspacing="0" id="troop_info" class="transparent">
        ...
        for ($i = 31; $i <= 40; $i++) {
            if($unit['u'.($i%30)]){
                echo '<tr>';
                echo '<td class="ico"><img class="unit u'.$i.'" src="img/x.gif" alt="" title="" /></td>';
                echo '<td class="val">'.$unit['u'.($i%30)].'</td>';
                echo '<td class="desc">'.${"U".$i}.'</td>';
                echo '</tr>';
            }else{
                $a = $a+1;
            }
...
<table cellpadding="0" cellspacing="0" id="village_info" class="transparent">
    <tbody>
        <tr>
          <th>Distance:</th>            
          <td><?php echo $distance;?> fields</td>
        </tr>
    </tbody>
</table>
<div class="instantTabs">
	<div class="contentNavi tabNavi tabSubWrapper">
		<div title="" class="container active">
			<div class="background-start">&nbsp;</div>
			<div class="background-end">&nbsp;</div>
			<div class="content">
				<a id="button<?php echo $toWref;?>1" href="#" class="tabItem"><font size="2"><?=logint412?></a>
			</div>
		</div>
		<script type="text/javascript">
			if ($('#button<?php echo $toWref;?>1')) {
				$('#button<?php echo $toWref;?>1').live('click', function () {
					window.fireEvent('tabClicked', [this, {"class":"active","title":false,"target":false,"id":"button<?php echo $toWref;?>1","href":"#","onclick":false,"enabled":true,"text":"Reports","dialog":false,"plusDialog":false,"goldclubDialog":false,"containerId":"","buttonIdentifier":"button577d54768e234"}]);
				});
			}
		</script>
		...
		<div title="" class="container normal">
			...
			<div class="background-end">&nbsp;</div>
			<div class="content">
				<a id="button<?php echo $toWref;?>2" href="#" class="tabItem"><font size="2">Surrounding</a>
			</div>
		</div>
		<script type="text/javascript">
			if ($('#button<?php echo $toWref;?>2')) {
				$('#button<?php echo $toWref;?>2').live('click', function () {
					window.fireEvent('tabClicked', [this, {"class":"normal","title":false,"target":false,"id":"button<?php echo $toWref;?>2","href":"#","onclick":false,"enabled":true,"text":"Surrounding","dialog":false,"plusDialog":false,"goldclubDialog":false,"containerId":"","buttonIdentifier":"button577d54768e28e"}]);
				});
			}
		</script>   
		<div class="clear"></div>
	...
	<div class="tabContainer">
		<table cellpadding="1" cellspacing="1" id="troop_info" class="rep transparent">
			...
			<?php if($query != 0){
				...
				?>			
			<?php  }else { ?>		
				<tr><td>No information<br>available!</td></tr>
			<?php  } ?>							
			</tbody>
		</table>		
	</div>
	<div class="tabContainer reports hide">		
		<table cellpadding="1" cellspacing="1" id="troop_info" class="reportSurround transparent">
			<tbody>
			<?php if(count($surrounding) != 0){
					$ownerId = $basearray['owner'];
					$name = $database->getVillageField($toWref, "name");
					if($ownerId!=2){
						$user = $database->getUserField($ownerId,'username',0);
					}
					else{ $user = "Nature";}
					$type = $row['ntype'];
					foreach($surrounding as $row){
						echo "<tr><td title=\"A fight took place in <a href='karte.php?d=".$toWref."'>".$name." ".$to['x']."|".$to['y']."</a> of <a href='spieler.php?uid=".$ownerId."'>".$user."</a>.\">";
						echo "<div class=\"reportIcon fight\"></div>";
						$date = $generator->procMtime($row['time']);
						echo $date[0]." ".date('H:i',$row['time']);
						echo "</td></tr>";
					}
				?>			
			<?php  }else { ?>		
...
<script type="text/javascript">
		window.addEvent('domready', function() {
			var instantTabs = new Travian.Game.InstantTabs;
			instantTabs.initialize();
		});
</script>
</div>
<div class="clear"></div>
