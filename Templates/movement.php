...
for($i=0;$i<$aantal4;$i++){
	if(($aantal55[$i]['attack_type']==1) or ($aantal55[$i]['attack_type']==3) or ($aantal55[$i]['attack_type']==4)) { $lala -= 1; }
}
$aantal = $aantal+$lala;
if($aantal > 0){
...
if($aantal > 0){
   ...
   if($countoas > 0){
	...
	echo '<tr><td class="typ"><a href="build.php?id=39"><img src="img/x.gif" class="'.$action.'" alt="'.$title.'" title="'.$title.'" /></a><span class="'.$aclass.'">&raquo;</span></td>
	<td><div class="mov"><span class="'.$aclass.'">'.$countoas.'&nbsp;'.$short.'</span></div><div class="dur_r">in&nbsp;<span id="timer'.$timer.'">'.$generator->getTimeFormat(min($NextArrival7)-time()).'</span>&nbsp;'.HOURS.'</div></div></td></tr>';
	$timer += 1;
}
