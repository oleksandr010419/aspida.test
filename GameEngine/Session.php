...
// Check if the 'lang' parameter is set in the query string
if(!empty($_GET['lang'])){
    if($_GET['lang']=='gr'){
        setcookie('lang','gr');
        $_COOKIE['lang']='gr';
        ...
        setcookie('lang', 'en');
        $_COOKIE['lang'] = 'en';
    } elseif ($_GET['lang'] == 'tr') { // Add Turkish language ('tr') support
        setcookie('lang', 'tr');
        $_COOKIE['lang'] = 'tr';
    }
}
...
if ((!isset($_COOKIE['lang']) || empty($_COOKIE['lang'])) && isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
    ...
    if (in_array($language, array('gr', 'ua', 'be'))) {
        setcookie('lang', 'gr');
        $language = $_COOKIE['lang'] = 'gr';
    } else {
        setcookie('lang', 'en');
...
// Add 'tr' (Turkish) to the list of supported languages
if(!in_array($language, array('gr', 'en', 'tr'))) {
    setcookie('lang', 'en');
    $language = $_COOKIE['lang'] = 'en';
}
