...
class funct {
  ...
  function Act2($post){
	global $admin;
	  ...
	  break;
	  case 'addBan':
		if($post['time']){$end = time()+$post['time']; }else{$end = '';}

		  if(!is_numeric($post['uid'])){
		  $post['uid'] = $admin->getUserField(addslashes($post['uid']),'id',1);
		  }

		$admin->AddBan($post['uid'],$end,$post['reason']);
		//add ban
	  break;
	  case 'delBan':
		$admin->DelBan($post['uid'],$post['id']);
		//remove ban
	  break;
	  case 'punish':
		$admin->Punish($post);
