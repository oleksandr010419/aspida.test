...

{
    ...
    public function procRankArray()
    {
        ...
        $q = "SELECT
            ...
            users.access,
            users.username,
            users.IsoCountryCode,
            users.alliance,
            (SELECT SUM(vdata.pop) FROM vdata WHERE vdata.owner = userid) AS totalpop,
            (SELECT COUNT(vdata.wref) FROM vdata WHERE vdata.owner = userid AND type != 99) AS totalvillages,
            (SELECT alidata.tag FROM alidata, users WHERE alidata.id = users.alliance AND users.id = userid) AS allitag
            FROM users
            WHERE users.access <> 8
            ORDER BY totalpop DESC, userid DESC";
        $datas = $database->query($q);

        foreach ($datas as $thisRow) {
            $holder[] = $thisRow;
        }
...

    {
        ...
        $q = "SELECT users.id userid, users.tribe tribe, users.username username, users.IsoCountryCode, users.alliance alliance, (
        ...
        foreach ($datas as $result) {
            ...
            $value['IsoCountryCode'] = $result['IsoCountryCode'];
...

    {
        ...
        $q = "SELECT users.id userid, users.username username, users.IsoCountryCode, users.apall,   (
        ...
        foreach ($datas as $row) {
            ...
            $value['IsoCountryCode'] = $row['IsoCountryCode'];
            $value['id'] = $row['userid'];
    ...
    private function procCountryRankArray ()
    {
        global $database;

        // Skip user 3 for the country rankings, because that's Natars Empire (the game's backstory)
        $sqlGetRankings = 'SELECT
            c.Country,
            c.IsoCountryCode,
            COUNT(DISTINCT u.id) AS Users,
            COUNT(DISTINCT vd.wref) AS Villages,
            SUM(vd.pop) AS TotalPop,
            (COUNT(DISTINCT u.id) + (COUNT(vd.wref) * 2) + FLOOR(SUM(vd.pop) / 1000)) AS Rank
            FROM users u
            INNER JOIN CountryTbl c ON u.IsoCountryCode = c.IsoCountryCode
            LEFT JOIN vdata vd ON u.id = vd.owner AND vd.type != 99
            WHERE u.id != 3
            GROUP BY c.IsoCountryCode
            ORDER BY Rank DESC';
        $datas = $database->query($sqlGetRankings);
...

    {
        ...
        $q = "SELECT users.id userid, users.username username, users.IsoCountryCode, users.dpall,  (
