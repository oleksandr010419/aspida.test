...
class Village {
    ...
    public $type;
    public $coor = array();
    public $awood,$aclay,$airon,$acrop,$pop,$maxstore,$maxcrop,$cp,$tech,$oasisowned,$currentcel,$starvupdate,$loyalty,$allcrop,$enforce;
    ...
    private function LoadTown()
    {
        ...
        $this->pop = $info['pop'];
        $infvil=array('tribe'=>$session->tribe,'loyalty'=>$info['loyalty'],'pop'=>$info['pop'],'lastupdate'=>$info['lastupdate'],'maxstore'=>$info['maxstore'],'maxcrop'=>$info['maxcrop'],'uid'=>$info['owner'],'lastup'=>$session->lastupdate);
        ...
        $this->awood = (int)$procinf[0][0];
        $this->aclay = (int)$procinf[0][1];
        $this->airon = (int)$procinf[0][2];
        $this->acrop = (int)$procinf[0][3];
		...
		if($this->acrop<0){
			$this->starvationInVil();
		}
		if(WW_PLAN > time()){
			$database->addInitialBonus($database->getTotalPop($session->uid));
		}
		checkVillageStatus($this->wid);
    }
                            ...
                            if($killunits > 0){
                                ...
                                if (isset($enf)){
                                    ...
                                    if($killunits < $maxcount){
                                        ...
                                        $database->modifyUnit($this->wid, array($maxtype), array($killunits), 0, "Units starve in village");
                                        if($maxutype == "hero"){
 ...
 global $session;
        ...
        if(count($enfoarray) > 0) {
            ...
            foreach($enfoarray as $enforce) {
				if ($enforce['tribe'] == NULL)
					$enforce['tribe'] = 4;
                for($i=1;$i<=10;$i++) {
					if ((($enforce['tribe']-1)*10+$i) < 0){
						//var_dump($enforce);
					}
    ...
    function VillageControl()
    {
        ...
        else  if (!$this->capital)
        {
            for($i=1;$i<=18;$i++)
            {
                if($this->resarray['f'.$i] > 10)
                {
                    $katoki="f".$i."";
                    $database->query("UPDATE fdata SET ".$katoki."='10' WHERE vref='".$this->wid."' ");
                }
            }
        }
