...
if($_POST && $_POST['a']=='inventory'){
	...
	elseif($itemData['btype']==4){
		...
		if($gi['rightHand']!=0){
			...
			$type = $id['type'];
			include dirname(__FILE__)."/Data/alt.php";
        ...
        $database->modifyHero2("itempower",$itempower,$uid,1);
	...
	if($data['amount'] <= $itemData['num']-$itemData['type']){ //если число введеных повязок меньше или равно их реальном кол-ву
		...
		/*if($gi['bag']!=0){
			$id = $database->getHeroItemID2($uid, 7, $gi['bag']);
	...
	elseif($itemData['btype']==8 && $data['amount']>0){ //походу воскрешает немного сдохших войск 33%
		if($data['amount'] <= $itemData['num']){
			...
			$database->editHeroType($itemData['id'], $data['amount'], 1);
			/*if($gi['bag']!=0){
				$id = $database->getHeroItemID2($uid, 8, $gi['bag']);
				$database->editProcItem($id['id'], 0, $uid);
			}*/
			$database->setHeroInventory($uid, 'bag', $itemData['id']);
		}
	...
	elseif($itemData['btype']==9 && $data['amount']>0){ //клетки на зверье
		if($data['amount'] <= $itemData['num']){
	...
	elseif($itemData['btype']==10 && $data['amount']>0  && $session->heroD['dead']!=1){ //опыт геру свитками!
		if($data['amount'] <= $itemData['num']){
			$value = ((int)$data['amount']*10);
			if($data['amount'] < $itemData['num']){
				$database->modifyHeroXp('experience', $value, $uid, 1);
	...
	elseif($itemData['btype']==11 && $data['amount']>0 && $session->heroD['dead']!=1){//хилим героя
	...
	elseif($itemData['btype']==12){//воскрешаем героя
        ...
        foreach($session->villages as $myvill){
            $hero+=$hero_model->HeroNotInVil($myvill); // check if hero is not in village (come back from attack , raid , etc.)
        }
		...
		if(!$hero){// && !$session->heroD['revivetime']
			if($session->heroD['dead']!=0){
				...
				$database->editProcItem($itemData['id'], 1, $uid);
				$database->modifyUnit($village->wid, array(11), array(1), 1,"Hero being revived by bucket");
			}
		}
	}

	elseif($itemData['btype']==13 && $session->heroD['dead']!=1 && $session->heroD['level'] > 0){//скидываем очки героя
	...
	if($itemData['btype'] >= 7){
		if($gi['bag']!=0){
			...
			$database->editProcItem($id['id'], 0, $uid);
            $database->editHeroType($id['id'], $id['type'], 0);
			//$database->editHeroNum($id['id'], $id['amount'], 1);
		}
	}
}
