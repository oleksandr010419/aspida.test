<?php
$time = time() - (60*5);
$sql = "SELECT * FROM ".TB_PREFIX."users where true ORDER BY id ASC $limit";
...
<?php
	...
	$time = time() - (60*5);
	$sql2 = "SELECT * FROM ".TB_PREFIX."users where true ORDER BY id ASC $limit";

if($query>0){
	$result = $database->query($sql2);
	foreach($result as $row){
		$uid = $row['id'];
		$sql3 = "SELECT * FROM ".TB_PREFIX."vdata where owner = $uid";
		$vil = $database->query($sql3);
		$totalpop = 0;
