...
if(isset($id))
{
	...
	if($village and $user)
	{
		...
		<link href="../img/admin/admin.css" rel="stylesheet" type="text/css">
		<br />
			...
			<tbody>
				...
				<tr>
					...
					<td colspan="2"><?php echo $village['pop'];?></td>
								...
								<tr>
					<td>ЕК <a href="admin.php?action=recountCp&did=<?php echo $_GET['did']; ?>"><?php echo $refreshicon; ?></a></td>
		...
		<table id="member">
			...
			<tbody>
              ...
              <a href="admin.php?p=villagelog&did=<?php echo $_GET['did']; ?>">Resource Transactions</a>
		...
		<table id="member" cellpadding="1" cellspacing="1" >
			...
			<tbody>
				<?php
					...
					if($isthere >0)
					{
						...
						{
							$oasiswref = $row['wref'];
							$oasisx = $database->single("SELECT x FROM wdata WHERE id = ".$oasiswref.""), 0);
							$oasisy = $database->single("SELECT y FROM wdata WHERE id = ".$oasiswref.""), 0);
         ...
         <?php
		if($_SESSION['access'] == 9){ ?><a href="admin.php?p=doublelog&did=<?php echo $_GET['did']; ?>"> Troops in Villages</a>

		<?php  }
		if($_SESSION['access'] == 9){ include('troops.tpl');
			     include('reinforlog.tpl');
