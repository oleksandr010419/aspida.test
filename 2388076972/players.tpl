<?php
$time = time() - (60*5);
$sql = $database->query("SELECT * FROM ".TB_PREFIX."users where true ORDER BY id ASC $limit");
$query = count($sql);
...
<?php
	...
	if ($page == "1" && $lastPage == "1"){
		...
		$paginationDisplay .=  '<img alt="صفحه قبل" src="../img/x.gif" class="previous disabled">';
		$paginationDisplay .= $centerPages;
		$paginationDisplay .=  '<a class="next" href="index.php?p=players&page=' . $nextPage . '"><img alt="صفحه بعد" src="../img/x.gif"></a> ';
		$paginationDisplay .=  '<a class="last" href="index.php?p=players&page=' . $lastPage . '"><img alt="صفحه آخر" src="../img/x.gif"></a>';
	
	}elseif ($page != "1" && $page != $lastPage){
		$paginationDisplay .=  '<a class="first" href="index.php?p=players&page=1"><img alt="صفحه اول" src="../img/x.gif"></a> ';
		$paginationDisplay .=  '<a class="previous" href="index.php?p=players&page=' . $previous . '"><img alt="صفحه قبل" src="../img/x.gif"></a>';
		$paginationDisplay .= $centerPages;
		$paginationDisplay .=  '<a class="next" href="index.php?p=players&page=' . $nextPage . '"><img alt="صفحه بعد" src="../img/x.gif"></a> ';
		$paginationDisplay .=  '<a class="last" href="index.php?p=players&page=' . $lastPage . '"><img alt="صفحه آخر" src="../img/x.gif"></a>';
	...
	$time = time() - (60*5);
	$sql2 = $database->query("SELECT * FROM ".TB_PREFIX."users where true ORDER BY id ASC $limit");

if($query>0){
	foreach($sql2 as $row){
		$uid = $row['id'];
		$vil = $database->query("SELECT * FROM ".TB_PREFIX."vdata where owner = $uid");
		$totalpop = 0;
	...
	echo '<tr><td colspan="8" align="center">هیچ بازیکنی ثبت نام نکرده است</td></tr>';
} 
